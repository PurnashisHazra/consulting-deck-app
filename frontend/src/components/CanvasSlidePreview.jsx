    ...
    if (slide.layout && Array.isArray(slide.sections)) {
      return slide.sections.map((section, idx) => {
        ...
        if (!content || (typeof content === 'string' && content.trim() === '')) {
          content = `No AI data for this section. Example: ${section.title || 'Section'} content.`;
        }
        // Chart Data
              ...
              {!["chart","frameworks","keyPoints","takeaway"].includes(item.type) && (
                <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm flex flex-col items-start h-full w-full overflow-auto" style={{justifyContent: 'flex-start', minHeight: 0, height: '100%', maxHeight: '100%'}}>
                  ...
                  <div className="mb-4 w-full">
                    <h3 className="text-sm font-semibold text-gray-800 uppercase tracking-wide mb-3">{item.title || item.type || "SECTION"}</h3>
                    {(() => {
                      if (Array.isArray(item.data)) {
                        ...
                        if (points.length > 1) {
                          ...
                          return (
                            <span className="text-xs text-gray-700 leading-relaxed whitespace-pre-line">{item.data}</span>
                          );
                        }
                      } else {
                        return null;
                      ...
                      <div className="mt-4">
                        ...
                        <div className="flex flex-col gap-4">
                          {frameworksArr.map((fw, idx) => {
                            const fwData = item.frameworkData[fw] || item.frameworkData.find(f => f.framework === fw)?.data;
